 ...
 * 
 * The same as the standard MoveAction class, except for that it extends CustomMoveAction object, 
 * which changes the behavior of the moving ship.

 * @author amy, Nathan Popp, Dillon Harper
 */
public class CustomMoveToObjectAction extends CustomMoveAction {
	 ...
	 * @param goalObject
	 */
	public CustomMoveToObjectAction(Toroidal2DPhysics space, Position currentLocation, AbstractObject goalObject) {
		super(space, currentLocation, goalObject.getPosition());
		this.goalObject = goalObject;
		this.originalGoalLocation = goalObject.getPosition().deepCopy();
	}
	
	public CustomMoveToObjectAction(Toroidal2DPhysics space, Position currentLocation, AbstractObject goalObject, Vector2D targetVelocity) {
		super(space, currentLocation, goalObject.getPosition());
		this.goalObject = goalObject;
		this.originalGoalLocation = goalObject.getPosition().deepCopy();
	}
