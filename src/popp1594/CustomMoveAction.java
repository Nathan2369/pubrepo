 ...
 * 
 * This class is identical to the MoveAction that the professor provided, except that a ship will not slow down as it approaches its target
 * @author amy, Nathan Popp, Dillon Harper
 */
public class CustomMoveAction extends AbstractAction {
	/**
	 ...
	 */
	public CustomMoveAction(Toroidal2DPhysics space, Position currentLocation, 
...
//		float Kp = 0.16f;
	 ...
	 * However, do not slow down when approaching the target
	 */
	public Movement getMovement(Toroidal2DPhysics space, Ship ship) {
		...
		//aim to hit in direction between target and path
		Vector2D unitVel = space.findShortestDistanceVector(ship.getPosition(), targetLocation).getUnitVector();
		targetVelocity = unitVel.multiply(50);
		
		//Set the ship to hit the target at the speed of the target velocity, in the direction that the
		//ship is facing. 
